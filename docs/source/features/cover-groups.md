...
!!! info
    Each `cover.` entity here refers to the **automatically created area group**, such as `cover.living_room` if your living room has covers. The actual `entity_id` generated by Adaptive Areas are much larger to prevent conflicts. You can find the specific `entity_id` via the UI on the entity's properties.
