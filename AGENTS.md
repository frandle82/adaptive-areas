...
Area Evaluation is intrinsic to every regular indoor Area and retains the RC4 Environment entity/unique-ID prefix for migration safety. It is not a selectable feature. It evaluates compatible, non-excluded sources and exposes category-aware thermal assessment, psychrometric values, mould risk and quality, time-weighted air-quality severity and coverage, source provenance, window advice, and ventilation/circulation fan requests; it never controls devices directly. General exclusions are authoritative for local, explicit, and exterior-Area sources. Entity attributes may expose source IDs/names for traceability, but diagnostics expose only counts/modes. The centralized matrix and profiles in `helpers/environment.py` are the source of truth. Fan Control consumes requests only when explicit fan roles and Fan Control are configured; otherwise existing aggregate/setpoint behavior remains. Optional Room Usage is a regular-area basic option, disabled by default, based only on existing presence transitions, and provides cleaning recommendations without device control. Missing information remains unknown and must never be interpreted as healthy. Air-quality severity is distinct from ventilation need; pollutants must not automatically imply that outdoor ventilation is safe. WHO 24-hour inputs require elapsed-time weighting and adequate coverage. TVOC mass is precautionary only; generic VOC ppb/AQI scales are unclassified. Air-quality/ventilation needs outrank thermal efficiency, and ventilation-fan semantics remain distinct from circulation-fan semantics. Decisions remain privacy-safe and traceable. Manual Override remains the pre-existing Light Groups behavior only.
