...
User-facing `environment` feature is **Room Climate** / **Raumklima**. It is optional, evaluates numeric indoor climate/environment measurements, and retains stable internal key and entity IDs. Area Health owns generic health/safety binary-sensor aggregation; legacy Magic Areas Environmental Sensors functionality is not imported as active functionality. Room Usage is independent optional feature based only on existing presence transitions and exposes cleaning recommendations without device control. Optional features are enabled through feature selection; enabled configurable features appear in options menu; disabled features instantiate no runtime components. Primary Area temperature, primary Area humidity, and room category remain fundamental General settings. General exclusions are authoritative. Missing information remains unknown. Air-quality severity remains distinct from ventilation need. WHO 24-hour inputs require elapsed-time weighting and adequate coverage. TVOC mass is precautionary only; generic VOC ppb/AQI scales are unclassified. Fan semantics and existing Manual Override behavior remain separate.
