...
The Environment Engine evaluates available environmental inputs and exposes assessment, derived thermal values, mould risk, air-quality severity, window advice, and ventilation/circulation fan requests; it never controls devices directly. The centralized matrix in `helpers/environment.py` is the source of truth for pollutant units and bands. Existing Fan Control consumes requests only when Environment Monitoring and Fan Control are enabled. Optional Room Usage is a regular-area basic option, disabled by default, based only on existing presence transitions, and provides cleaning recommendations without device control. Partial sensor availability is required: missing information remains unknown and must never be interpreted as healthy. Air-quality severity is distinct from ventilation need; PM or other pollutant severity must not automatically imply that outdoor ventilation is safe. Air-quality/ventilation needs outrank thermal efficiency, and ventilation-fan semantics must remain distinct from circulation-fan semantics. Environment decisions must remain privacy-safe and traceable. Manual Override remains the pre-existing Light Groups behavior only.
