...
from custom_components.adaptive_areas.const import (
    ...
    AdaptiveAreasFeatureInfoWaspInABox,
...
class AreaWaspInABoxBinarySensor(AdaptiveEntity, BinarySensorEntity):
    """Wasp In The Box logic tracking sensor for the area."""

    feature_info = AdaptiveAreasFeatureInfoWaspInABox()

    def __init__(self, area: AdaptiveArea) -> None:
        """Initialize the area presence binary sensor."""

        AdaptiveEntity.__init__(self, area, domain=BINARY_SENSOR_DOMAIN)
        BinarySensorEntity.__init__(self)

        self._delay: int = self.area.feature_config(
            AdaptiveAreasFeatures.WASP_IN_A_BOX
    ...
    async def async_added_to_hass(self) -> None:
        ...
        for device_class in wasp_device_classes:
            dc_entity_id = f"{BINARY_SENSOR_DOMAIN}.adaptive_areas_aggregates_{self.area.slug}_aggregate_{device_class}"
